...
class AccountDetailViewModel : ViewModel() {
        ...
        viewModelScope.launch {
            try {
                ...
                if (!transaction.transferGroupId.isNullOrBlank() && !transaction.targetAccountId.isNullOrBlank()) {
                    ...
                    val otherIsCrypto = otherAccountDoc.get<String>("type") == "CRYPTO"
                    val currentAccountDoc = db.collection("accounts").document(accountId).get()
                    val currentIsCrypto = currentAccountDoc.get<String>("type") == "CRYPTO"

                    val otherTxSnapshot = otherAccountRef
                        .where("transferGroupId", equalTo = transaction.transferGroupId)
                        .get()
    ...
    fun performTransfer(
        ...
        viewModelScope.launch {
            try {
                ...
                val sourceIsCrypto = sourceAccDoc.get<String>("type") == "CRYPTO"
                val targetIsCrypto = targetAccDoc.get<String>("type") == "CRYPTO"
            ...
            try {
                ...
                when (scope) {
                    ...
                    RecurrenceEditScope.THIS_ONLY -> {
                        ...
                        val futureInstant = DateTimeUtils.addMonths(effectiveInstant, 1)
                        val futureTimestamp = Timestamp(futureInstant.toEpochSeconds(), (futureInstant.nanosecondsOfSecond))
        ...
        viewModelScope.launch {
            try {
                ...
                for (i in 0 until durationMonths) {
                    ...
                    val date = Timestamp(iterInstant.toEpochSeconds(), (iterInstant.nanosecondsOfSecond))
                    ...
                    if (account.linkedAccountId != null && iterInstant >= currentMonthStart) {
    ...
    fun updateLoanMetadata(accountId: String, totalCapital: Double, accountName: String) {
        viewModelScope.launch {
            try {
                ...
                val oldAmount = accountData.get<Double>("totalAmount") ?: 0.0
                ...
                for (doc in selfTxs.documents) {
                    ...
                    val currentRemaining = doc.get<Double>("remainingDebt")
                ...
                val linkedId = accountData.get<String>("linkedAccountId")
                ...
                _importStatus.value = "Erreur mise à jour : ${e.message}"
            }
        }
    }
            ...
            try {
                ...
                rows.drop(startRowIdx).forEachIndexed { index, pdfRow ->
                    ...
                    if (instant != null && amount > 0) {
                        ...
                        val timestamp = Timestamp(instant.toEpochSeconds(), instant.nanosecondsOfSecond)
                        ...
                        if (linkedAccountId != null && instant >= currentMonthStart) {
                            val debitTx = Transaction(
                                title = accountName,
                                amount = -amount,
                                type = "EXPENSE",
    ...
    private fun parseFlexibleDate(text: String): Instant? {
        ...
        try {
            ...
            return LocalDateTime(year, month, day, 0, 0).toInstant(TimeZone.currentSystemDefault())
    ...
    private val Instant.nanosecondsOfSecond: Int get() = this.nanosecondsOfSecond
